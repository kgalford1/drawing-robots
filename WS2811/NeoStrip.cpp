...
void NeoStrip::setPixels(int p, int n, const int *colors, const int *bright)
{
	int r, g, b;
    for (int i = 0; i < n; i++)
    {
        r = ((colors[i] & 0xFF0000) * bright[i]) >> 16;
        g = ((colors[i] & 0x00FF00) * bright[i]) >> 8;
        b = ((colors[i] & 0x0000FF) * bright[i]);
        setPixel(p+i, r, g, b);
    }
}
...
void NeoStrip::setColor(int color)
{
    for (int i = 0; i < N; i++)
	setPixel(i, color);
}
